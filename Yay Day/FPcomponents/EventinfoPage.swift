...
struct EventInfoPage: View {
        ...
        ScrollView {
            VStack(alignment: .center) {
                ...
                VStack(alignment: .leading, spacing: 15) {
                    HStack(alignment: .top) {
                        ...
                        VStack(alignment: .trailing, spacing: 15) {
                            HStack {
                                ...
                                Text({
                                    if let userLocation = userLocation {
                                        let eventLocation = CLLocation(latitude: event.address.coordinates[0], longitude: event.address.coordinates[1])
                    ...
                    Button(action: {
                        if let url = URL(string: event.webUrl), UIApplication.shared.canOpenURL(url) {
    ...
    // Helper methods
      func createEventShareText() -> String {
          let eventLink = "yayday://event/\(event.id)"
          return "Check out this event on YayDay! \(eventLink)"
      }
  ...
  struct ActivityViewController: UIViewControllerRepresentable {
    ...
    func copyEventLinkToClipboard() {
            let eventLink = "yayday://event/\(event.id)"
            UIPasteboard.general.string = eventLink
            print("Link copied to clipboard: \(eventLink)")
        }
    ...
    func openMapLocation(userLatitude: Double, userLongitude: Double) {
        ...
        let mapUrlString = "http://maps.apple.com/?saddr=\(userLatitude),\(userLongitude)&daddr=\(eventLatitude),\(eventLongitude)"
